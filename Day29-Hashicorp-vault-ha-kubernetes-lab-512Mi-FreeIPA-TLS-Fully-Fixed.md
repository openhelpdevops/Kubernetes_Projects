...
## Why use three Vault nodes?

Three voting members tolerate one unavailable member while retaining the two-member majority required for Raft quorum.

## Why use Integrated Storage?

Integrated Storage is built into Vault, provides HA and leader election, and supports Raft snapshots without requiring an external Consul cluster.

## Does Ceph RBD replace Raft?

No. Raft is Vault’s storage engine and consistency layer. Ceph RBD provides the durable block volume on which each Raft member stores its local database.

## Why does every Vault pod need a separate PVC?

Each Raft node owns its own local Raft data. Sharing one RBD filesystem between multiple Vault pods would violate this storage model and create corruption and availability risks.

## Why use `ReadWriteOnce`?

A Ceph RBD image is mounted as a block device to one Kubernetes node at a time. This matches the StatefulSet model of one dedicated volume per Vault pod.

## Why use a LoadBalancer that selects only the active node?

Vault standby nodes can forward many requests, but selecting the active service gives external clients a stable endpoint that tracks the current leader through Kubernetes service registration.

## Why is TLS required?

Vault transports credentials, tokens, unseal operations, and sensitive API data. TLS protects confidentiality and authenticates the Vault endpoint.

## What happens when the Vault leader fails?

The remaining Raft members elect a new leader if quorum remains. Kubernetes updates the active pod label, and the LoadBalancer service endpoint moves to the new leader.

## Why does a restarted node become sealed?

With Shamir seal, Vault does not retain the root key in memory across restarts. The unseal threshold must be supplied again. Auto-unseal removes this manual operational step.

## Why use Kubernetes authentication?

A pod can prove its identity with its Kubernetes service account token. Vault validates that identity and issues a short-lived Vault token attached to tightly scoped policies.

## What does the Vault Agent Injector do?

It is a mutating admission webhook. It adds Vault Agent init and sidecar containers plus an in-memory shared volume, authenticates to Vault, and renders secrets into files for the application.

## Why are dynamic database credentials better than static passwords?

Vault creates unique, short-lived users for each lease and revokes them automatically. This reduces password sharing, limits exposure time, improves attribution, and removes hardcoded long-lived credentials.

## What is the difference between token TTL and database lease TTL?

The Vault token TTL controls how long the client can call Vault. The database lease TTL controls how long the generated MySQL username and password remain valid.

## Why enable audit logging?

Vault audit logs record requests and responses in a security-conscious format, providing evidence of who accessed which paths and when.

## How do you back up Integrated Storage?

Use `vault operator raft snapshot save`, copy the snapshot to protected external storage, and regularly test restoration in an isolated environment.

## What are the most important Vault alerts?

Sealed node, loss of quorum, no active leader, failed health checks, authentication failure spikes, request latency, Raft storage errors, certificate expiry, and Raft/audit PVC capacity.
